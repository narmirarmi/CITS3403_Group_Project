...
def create_images(users):
    images = []
    for user in users:
        for _ in range(random.randint(0, 3)):  # Each user will have 1-3 images (maybe we should make it 0-x but for now 1-3 is fine
...
def create_votes(users, images):
    votes = []
    for image in images:
        sampled_users = random.sample(users, random.randint(1, len(users)))  # Each image gets 1 to all users voting on it
...
def create_follows(users):
    ...
    for user in users:
        ...
        sampled_users = random.sample(other_users, random.randint(1, len(other_users) // 2))  # Each user follows half of the other users
